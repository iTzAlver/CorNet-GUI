...
from ._utils import HoverButton, ColorStyles, layers_help
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg, NavigationToolbar2Tk
from PIL import Image

from basenet import BaseNetCompiler, BaseNetDatabase, BaseNetModel
from basenet import PREBUILT_LOSSES, KERAS_LOSSES, PREBUILT_LAYERS, KERAS_LIST_LAYERS, KERAS_OPTIMIZERS
# PREBUILT_OPTIMIZERS
...
# -----------------------------------------------------------
from .__special__ import __gui_ico_path__, __logfile_path__, __language_models_path__, __render_model_path__, \
    __models_location__, __database_location__, __models_lists_path__

ICO_LOCATION = __gui_ico_path__
LOGFILE_PATH = __logfile_path__
LMS_PATH = __language_models_path__
DRAW_MODEL_PATH = __render_model_path__
MODEL_LOCATION = __models_location__
DATABASE_LOCATION = __database_location__
MODEL_LISTS_PATH = __models_lists_path__
# -----------------------------------------------------------
...
class MainWindow:
    def __init__(self, master):
        ...
        # Tensorflow devices:
        self.devices = BaseNetCompiler.show_devs()
        self.devices_list = []
        self.devices_role: dict[str, str] = {}
        self.device_roles = ['Idle', 'Train', 'Feed']
        for dev, role_init in self.devices.items():
            self.devices_list.append(dev)
            self.devices_role[dev] = self.device_roles[0]
        for dev in self.devices_role:
            if 'GPU' in dev:
                self.devices_role[dev] = self.device_roles[1]
                break
        # Training variables:
        ...
        self.typeoflayers = KERAS_LIST_LAYERS
        self.typeoflayers.extend(PREBUILT_LAYERS)
        self.typeoflayers.extend(add_pipeline)
        self.losses = KERAS_LOSSES
        self.losses.extend(PREBUILT_LOSSES)
        self.metrics = ['accuracy', 'mse']
        self.optimizers = KERAS_OPTIMIZERS
    ...
    def import_database(self):
        ...
        dbpath = filedialog.askopenfilename(filetypes=[('Database files', '*.db')], initialdir=DATABASE_LOCATION)
        if dbpath:
            database = BaseNetDatabase.load(dbpath)
    ...
    def export_model(self):
        ...
        filename = filedialog.asksaveasfilename(filetypes=[('Binary files', '*')], initialdir=MODEL_LISTS_PATH)
        if filename:
        ...
        Report(image_path=f'{__render_model_path__}/{self.model.name}.png',
               model=self.model,
               db=self.database,
               history={'loss': self.history[self.model.name], 'val_loss': self.eval[self.model.name]})

    def import_model(self):
        # Import the deep learning model.
        filename = filedialog.askopenfilename(filetypes=[('Binary files', '*')], initialdir=MODEL_LISTS_PATH)
        if filename:
            with open(filename, 'rb') as file:
                self.current_model_list = pickle.load(file)
                self.listoflayers["values"] = [val[0] for val in self.current_model_list]
            self.model = BaseNetModel.load(f'{MODEL_LOCATION}/{filename.split("/")[-1]}.h5')
    ...
    def train_static(self):
        ...
        if self.running:
            ...
            self.running = True
            self.lowrite(_text=f'Training the current model. Batch size: {self.database.batch_size}.', cat='Info')

            self.model.add_database(self.database)
            my_results = self.model.fit(avoid_lock=True, epochs=1000, tensorboard=False)

            last_len = 0
            while my_results.is_training:
                self.master.update_idletasks()
                self.master.update()
                history = my_results.get()
                this_len = len(history['loss'])
                if not self.running:
                    self.running = True
                    self.lowrite(f'Stopping training, waiting for last epoch.', cat='Info')
                    self.model.fit_stop()
                if this_len > last_len:
                    self.lowrite(f'Epoch finished, LOSS = {round(history["loss"][-1], 3)}.', cat='Info')
                    last_len = this_len
                    self.history[self.model.name].append(history['loss'][-1])
    ...
    def list_devices(self):
        ...
        for ix, dev in enumerate(self.devices_list):
            self.lowrite(f'\tDevice {ix}:\t{dev}\t\tRole: {self.devices_role[dev]}\n', cat='Intro')
    ...
    def draw_scheme(self, tf_draw=True):
        ...
        if tf_draw:
            self.model.print(DRAW_MODEL_PATH)
        else:
        ...
        png = Image.open(f'{DRAW_MODEL_PATH}/{self.model.name}.png')
    ...
    def _parse_compiler(self):
        ...
        for idx, layer in enumerate(layers):
            this_args = {}
            for ix, kwd in enumerate(kwds[idx]):
                if kwd is not None:
                    this_args = {kwd: args[ix]}
            layers_parsed.append({layer: (shapes[idx], this_args)})

        print(devices)
        mycompiler = BaseNetCompiler(io_shape=((self.throughput, self.throughput, 1), self.throughput),
                                     layers=layers_parsed,
                                     compile_options=compiler,
                                     devices=devices,
                                     verbose=True)
